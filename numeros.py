...
def eh_primo(n):
    ...
    """
    qtd_divisores = 0
    for i in range(1, n+1):    # i varia de 1 ate n
        if n % i == 0:
            qtd_divisores += 1
    if qtd_divisores == 2:
        return True
...
def eh_armstrong(n):
    ...
    for digito in digitos:
        soma += digito ** total_digitos

    return soma == n_inicial
...
def eh_quase_armstrong(n):
    ...
    return soma in [n_inicial-1, n_inicial+1]
...
def eh_perfeito(n):
    ...
    """
    soma = 0
    for i in range(1, n):   # i varia de 1 ate n-1
        if n % i == 0:
            soma += i
...
def lista_perfeitos(n):
    ...
    """
    numeros_perfeitos = []
    for i in range(2, n):   # i varia de 2 ate n-1
        if eh_perfeito(i):
